...
export default function Input({title, typeInput, placeholderText, classStyle}) {
  const {setEmail, setPassword, setConfirmPassword, setCiudad, setApellido, setDireccion, } = useContext(LoginContext)
  const handleInput = (e) =>{
    if(inputsTypes.includes(title)){
      ...
      else if(title.toUpperCase() == "NOMBRE") setNombre(e.target.value)
